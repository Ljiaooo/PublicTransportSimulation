...
class ControlFunc():
    ...
    def changeFocus(self, controllers, to):
        ...
        for controller in controllers:
            if (controller.route == cur and to != 0) or (cur == 0 and controller.route != to):
                controller.selectPolygon(0)
                controller.selectPolygon(1)
                controller.selectRoute(0)
                controller.selectRoute(1)
                controller.selectBuses(0)
                controller.selectBuses(1)
    ...
    def writeData(self,controllers):
        ...
        waitingNum = []
        for controller in controllers:
            waitingNum.append(sum(controller.waitingNum))
            waitingTime.append(sum(controller.waitingTime))
...
class SumoThread(Thread):
    def __init__(self, control_funcs, controllers, upP,upB,downP,downB):
    ...
    def run(self):
        ...
        while step < 86400:
            ...
            self.upP.sumo_update()
            self.downP.sumo_update()
            self.upB.sumo_update()
            self.downB.sumo_update()
