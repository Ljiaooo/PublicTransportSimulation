    ...
    def writeData(self, controllers):
        ...
        if self.cur!=self.globalView:
            ...
            for controller in controllers:
                ...
                total_waiting_num += sum(controller.waitingNum)
                total_waiting_time += sum(controller.waitingTime)
            globalVal.data["overview"].append(departed_num)
            globalVal.data["overview"].append(drving_bus)
            globalVal.data["overview"].append(total_waiting_num)
            globalVal.data["overview"].append(total_waiting_time)
