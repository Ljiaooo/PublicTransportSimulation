...
class Widget(QMainWindow, Ui_SumoController):
    ...
    def refresh_table(self):
        ...
        if index == 1:
            self.global_0.setText(str(globalVal.data["overview"][0]))
            self.global_1.setText(str(globalVal.data["overview"][1]))
            self.global_2.setText(str(globalVal.data["overview"][2]))
            self.global_3.setText(str(globalVal.data["overview"][3]))
        ...
        if index == 3:
            cnt = 0
            maxLen = len(globalVal.data["waiting_time"])
            for i in range(5):
                for j in range(8):
                    if cnt<maxLen:
